...
@section('content')
<td><a class="btn btn-info text-white" href="{{route("report.form" , ['carId' => $car->id])}}">ادیت گزارش</a></td>

<div class="card">
    <div class="card-header">
        <strong>گزارش کارشناسی خودرو</strong>
    </div>
    <div class="card-body">
        <table class="table">
            <tr>
                <td><strong>نام مشتری: </strong>{{$customer->name}}</td>
                <td><strong>خودرو: </strong>{{$car->name}}</td>
                <td><strong>تاریخ انجام گزارش: </strong>{{$date[0]}}</td>
                <td><a href="{{ route('download.pdf', ['carId' => $car->id]) }}" class="btn btn-primary">دانلود گزارش PDF</a></td>
            </tr>
        </table>
        <hr>
