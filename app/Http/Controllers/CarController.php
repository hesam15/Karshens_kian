    ...
    public function store(Request $request, $id){
        Cars::where('id', $id)->update([
            ...
            'vip_services' => [
                //تست رانندگی
                'driving_test' => [
                    'car_start' => $request->car_start,
                    'cv_joints' => $request->cv_joints,
                    'shocks' => $request->shocks,
                    'cabin_noise' => $request->cabin_noise,
                    'engine_vibration' => $request->engine_vibration,
                    'steering_system' => $request->steering_system,
                    'exhaust_smoke' => $request->exhaust_smoke,
                    'transmission_sound' => $request->transmission_sound,
                    'clutch_plate' => $request->clutch_plate,
                    ...
                    'idle_performance' => $request->idle_performance,
                    'steering_angle' => $request->steering_angle,
                    'exhaust_sound' => $request->exhaust_sound,
                ],

                //قولنامه و حقوقی
                'legal' => [
                    'military_legal_inquiry' => $request->military_legal_inquiry,
                    'traffic_violation_inquiry' => $request->traffic_violation_inquiry, 
                    'legal_contract_setup' => $request->legal_contract_setup,
                ],

                //ارتقاء ظاهری خودرو
                'upgrade' => [
                    'guaranteed_nanotechnology' => $request->guaranteed_nanotechnology,
                    'line_removal' => $request->line_removal,
                    'polishing' => $request->polishing,
                    'paint_protection' => $request->paint_protection,
                    'approved_bodywork' => $request->approved_bodywork,
                    'approved_painting' => $request->approved_painting,
                ],
            ],
            'report' => true,
        ]);
